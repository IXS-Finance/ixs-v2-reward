...
contract RewardSugar is IRewardSugar { 
    ...
    function _getEarned(
        ...
        require(voter.isGauge(_gauge), "Not a valid gauge");
    ...
    function _getFeeAndBribeVotingRewards(
        ...
        feeRewards = _getEarned(_gauge, _tokenId, feeVotingReward);
        bribeRewards = _getEarned(_gauge, _tokenId, bribeVotingReward);
    }
    ...
    function getFeeVotingRewards(
        ...
        address feeVotingReward = voter.gaugeToFees(_gauge);
        rewards = _getEarned(_gauge, _tokenId, feeVotingReward);
        tokens = _getPoolTokens(_gauge);
    }
    ...
    function getBribeVotingRewards(
        ...
        rewards = _getEarned(_gauge, _tokenId, bribeVotingReward);
        tokens = _getPoolTokens(_gauge);
    }
    ...
    function getFeeAndBribeVotingRewards(
        ...
        (feeRewards, bribeRewards) = _getFeeAndBribeVotingRewards(_gauge, _tokenId);
        tokens = _getPoolTokens(_gauge);
    }
    ...
    function getFeeAndBribeVotingRewardsForOneTokenIdAndMultipleGauges(
        ...
        for (uint256 i = 0; i < length; i++) {
            (feeRewards[i], bribeRewards[i]) = _getFeeAndBribeVotingRewards(_gauges[i], _tokenId);
            tokens[i] = _getPoolTokens(_gauges[i]);
        }
    }
}